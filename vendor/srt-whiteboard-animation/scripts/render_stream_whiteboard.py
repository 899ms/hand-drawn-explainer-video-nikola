...
class RegionStreamRenderer:
    ...
    def __init__(self, image_bgr: np.ndarray, annotation: dict, cfg: sr.Config,
                 hand_png: Path | None, bare_tip: bool, hand_follow: float = 1.0) -> None:
        self.cfg = cfg
        self.ann = annotation
        # 1.0 为立即跟随；较小值让手部以缓动追随笔尖，降低跨笔画时的视觉跳动。
        self.hand_follow = max(0.08, min(1.0, float(hand_follow)))
        self._hand_display_pos: tuple[int, int] | None = None
    ...
    def _snapshot_with_tip(self, px: int, py: int) -> np.ndarray:
        ...
        if self.tip is not None:
            if self._hand_display_pos is None:
                self._hand_display_pos = (px, py)
            else:
                old_x, old_y = self._hand_display_pos
                self._hand_display_pos = (
                    int(round(old_x + (px - old_x) * self.hand_follow)),
                    int(round(old_y + (py - old_y) * self.hand_follow)),
                )
            self.tip.stamp(snap, self._hand_display_pos[0], self._hand_display_pos[1])
        return snap

    def _hold(self, writer, frames: int) -> None:
        """没有可画像素时保持当前画布，不能缩短后续时间轴。"""
        if frames <= 0:
            return
        snap = self.drawn.astype(np.uint8)
...
def _parse_args(argv=None):
    ...
    p.add_argument("--hand-follow", type=float, default=1.0, help="手部追随笔尖比例(0.08-1.0；默认1.0，0.35更平滑)")
    p.add_argument("--hand-height", type=int, default=None,
                   help="手部素材输出高度；16:9 复杂场景可用 260-340，默认沿用 493")
...
def _build_cfg(args) -> sr.Config:
    ...
    if args.hand_height is not None:
        if args.hand_height < 80:
            raise ValueError("--hand-height 不能小于 80")
...
def main(argv=None) -> int:
    ...
    errors, warnings = validate_annotation(annotation, (image_w, image_h))
    ...
    renderer = RegionStreamRenderer(image_bgr, annotation, cfg, hand_png, args.bare_tip, args.hand_follow)
