...
//
// Usage:
//   node scripts/animation-map.mjs <composition-dir> --cli <entry.mjs> --browser <chrome> \
//     [--frames N] [--out <dir>] [--min-duration S] [--width W] [--height H] [--fps N]

import { mkdir, writeFile } from "node:fs/promises";
import { resolve, join } from "node:path";

// Compatibility wrapper for inline HTML/SVG/GSAP projects only.
// The inspection logic below is from the installed HyperFrames Skill helper.
import {createRequire} from 'node:module';
import {pathToFileURL} from 'node:url';
import {readFile,access} from 'node:fs/promises';
async function createFileServer({projectDir}) {
 if(!args.cli||!args.browser)throw Error('Supply --cli <JS entrypoint> and --browser <executable>.');
 await access(resolve(args.cli));await access(resolve(args.browser));
 const file=join(projectDir,'index.html');const source=await readFile(file,'utf8');
 if(/data-composition-src|<video[\s>]/i.test(source))throw Error('Inline adapter does not support sub-compositions/video; use official CLI inspection.');
 return {url:pathToFileURL(file).href,close(){}};
}
async function createCaptureSession(url,out,config) {
 const req=createRequire(resolve(args.cli));
 const {default:puppeteer}=await import(pathToFileURL(req.resolve('puppeteer-core')).href);
 const browser=await puppeteer.launch({executablePath:resolve(args.browser),headless:true,args:['--disable-gpu']});
 const page=await browser.newPage();await page.setViewport({width:config.width,height:config.height});
 return {browser,page,url};
}
async function initializeSession(s){await s.page.goto(s.url);await s.page.evaluate(()=>document.fonts.ready);}
...
try {
  ...
  if (!Number.isFinite(duration) || duration <= 0 || !tweens.length) {
    throw Error('Missing duration or registered GSAP tweens; use official CLI inspection.');
  }
